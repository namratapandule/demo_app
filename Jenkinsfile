pipeline {
  ...
  stages {
    ...
    stage('deploy') {
      steps {
       sshpublisher(publishers: [sshpublisherDesc(configName: '',transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: '', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false,patternseperator: '[,]+', remoteDirectory: '', remoteDirectorySDF: false, removeprefix: 'dist/',sourceFiles: 'dist/Lending-PORTAL/**/*')],usePromotionTimestamp: false, useWorkspaceInPramotion: false, verbose: true)]) 
      }
    }
  }
}